...
export default function CafePOSPage() {
  ...
  const [checkoutMethod, setCheckoutMethod] = useState<"Cash" | "Chapa">(
    "Cash",
  );
  ...
  const addToTab = (product: any) => {
    if (!activeTab)
      return showToastMessage("Please select or create a tab first!");
  ...
  const removeFromTab = (productId: string) => {
    ...
    const newItems = currentItems
      ...
      .map((i: any) => ({ ...i.Product, quantity: i.Quantity }));
    syncTabItems(activeTab, newItems);
  };
  ...
  const processPayment = async () => {
    if (checkoutMethod === "Chapa") {
      ...
      const res = await fetch("/api/chapa/initialize", {
        ...
        body: JSON.stringify({
          amount,
          email: "cafecustomer@sifra.et", // Chapa requires an email
          firstName: "Cafe",
          lastName: "Customer",
          title: `Order ${activeTab.OrderID}`,
          description: `Payment for order ${activeTab.OrderID}`,
      ...
      if (res.ok) {
        ...
        setQrCodeData(
          `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(data.checkout_url)}`,
        );
        setPendingTxRef(data.tx_ref);
      } else {
        const errorData = await res.json();
        showToastMessage(
          errorData.error || "Failed to initialize Chapa payment.",
        );
      }
      return;
    }
  ...
  const calculateTotal = (tab: any) => {
    ...
    return tab.OrderItems.reduce(
      (acc: number, item: any) =>
        acc + item.Quantity * item.Product.SellingPrice,
      0,
    );
  };
  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] gap-6 overflow-hidden relative">
      ...
      {toast && (
        <div
          className={`absolute top-4 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-lg shadow-xl z-50 text-white font-bold transition-all duration-300 ${toast.type === "success" ? "bg-green-600" : "bg-red-500"}`}
        >
          {toast.message}
        </div>
      )}
      ...
      <div className="w-64 bg-white p-4 rounded-lg shadow flex flex-col">
        <button
          ...
          className="w-full bg-olive text-white py-3 rounded-lg font-bold hover:bg-olive-dark mb-4 transition-colors"
        >
          + New Order
        </button>
          ...
          {tabs.map((tab) => (
            ...
            >
              <div className="font-bold text-gray-900">
                {tab.OrderName || "Unknown Order"}
              </div>
              <div className="text-sm text-gray-500 mt-1">
                {calculateTotal(tab)} ETB {tab.OrderItems?.length || 0} items
              </div>
            </button>
          ))}
          {tabs.length === 0 && (
            <p className="text-gray-400 text-center mt-4">No active orders</p>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 bg-white p-6 rounded-lg shadow overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          {activeTab && (
            <span className="bg-olive/10 text-olive px-4 py-1 rounded-full font-bold">
              Editing: {activeTab.OrderName}
            </span>
          )}
        </div>
        <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
          {products.map((p) => (
            <button
              key={p.ProductID}
              onClick={() => addToTab(p)}
              className={`p-4 border rounded-xl text-left transition-all hover:border-gold hover:shadow-md cursor-pointer`}
            >
      ...
      <div className="w-80 bg-white p-6 rounded-lg shadow flex flex-col relative">
        ...
        <div className="flex-1 overflow-y-auto space-y-4">
          {!activeTab ? (
            ...
            activeTab.OrderItems.map((item: any, index: number) => (
              <div
              ...
              >
                <div>
                  ...
                  <div className="text-sm text-gray-500">
                    {item.Product.SellingPrice} ETB x {item.Quantity}
                  </div>
                </div>
                <div className="font-bold text-gray-900 flex items-center gap-4">
                  <span>{item.Product.SellingPrice * item.Quantity}</span>
                  <button
                    onClick={() => removeFromTab(item.ProductID)}
                    className="text-red-500 hover:text-red-700 text-sm font-bold bg-red-50 w-6 h-6 rounded-full flex items-center justify-center cursor-pointer"
                  >
                    ×
                  </button>
                </div>
          ...
          <div className="absolute inset-0 bg-white/95 backdrop-blur-sm z-10 flex flex-col p-6 rounded-lg border border-gray-200">
            ...
            {checkoutMethod === "Chapa" ? (
              <div className="flex-1 flex flex-col items-center justify-center">
                {!qrCodeData ? (
                  <button
                    ...
                    className="bg-[#00A859] text-white px-6 py-3 rounded-lg font-bold shadow-md hover:bg-[#008f4c]"
                  >
                    Generate Chapa Link
                  </button>
                ) : (
                  <div className="text-center">
                    <p className="font-bold text-gray-900 mb-4">
                      Customer: Scan to Pay
                    </p>
                    <img
                      src={qrCodeData}
                      alt="Chapa QR"
                      className="mx-auto border p-2 rounded-xl shadow-sm mb-6"
                    />
                    <button
                      onClick={verifyChapaPayment}
                      disabled={verifying}
                      className="w-full bg-olive text-white py-3 rounded-lg font-bold disabled:opacity-50"
                    >
              ...
              <div className="flex-1 flex flex-col justify-end">
                <button
                  onClick={processPayment}
                  className="w-full bg-olive text-white py-4 rounded-xl font-bold text-lg"
                >
            ...
            <button
              ...
              className="mt-4 w-full py-3 text-gray-500 font-bold hover:bg-gray-50 rounded-lg"
            >
              Cancel
            </button>
          </div>
      ...
      {showNewTabModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white p-8 rounded-xl shadow-xl w-96 max-w-[90%]">
            <h3 className="text-2xl font-bold mb-4 font-serif text-olive">
              Create New Order
            </h3>
            <form onSubmit={handleCreateTabSubmit}>
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Customer or Table Name
                </label>
                <input
                  type="text"
                  autoFocus
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-3 text-gray-600 font-bold border border-gray-200 rounded-lg hover:bg-gray-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-3 bg-olive text-white font-bold rounded-lg hover:bg-olive-dark"
                >
                  Create Order
                </button>
              </div>
